...
class Ui_Dialog(object):
    ...
    def ip2(self):
        ...
        while i<(self.entry+1):
            ...
            nor = np.genfromtxt(str(name), usecols=(1,), unpack=True)
            print nor
    ...
    def run(self):
        ...
        envi.save_image(str(self.name_op)+'.hdr',cimage,dtype=np.float32,ext = 'File',force=True,interleave="bil")
        time.sleep(0.500)
        self.w = Popup()
...
class Ui_MainWindow(object):
    def __init__(self):
        ...
        self.window.show()
        self.window.exec_()     #VERY VERY IMPORTANT
app = QtGui.QApplication(sys.argv)
app.setWindowIcon(QtGui.QIcon('Icon\hydas.png'))
myapp = Linear_Unmixing()
myapp.show()
sys.exit(app.exec_())
